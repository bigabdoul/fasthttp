...
const badRange = new RangeError("Status must be between 300 and 399 inclusive.");
...
class FastHttpResponse {
    ...
    download(path, filename, options, callback) {
        console.debug("download not implemented", path, filename, options, callback);
        throw notImplemented;
    }
    ...
    redirect(status, path) {
        ...
        if (arguments.length === 1) {
            ...
            if (typeof status === "number" && typeof path === "string") {
                ...
                if (code < 300 || code > 399) throw badRange;
            } else throw badArguments;
        } else throw badArguments;

        this.status(code).writeHeader("Location", path).end();
    }
