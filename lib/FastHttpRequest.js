...
class FastHttpRequest {
  ...
  set routePath(value) {
    if (this._routePath !== value) {
      ...
      if (value) {
        const keys = []
        this._regexp = pathToRegexp(value, keys);
        this._pathkeys = keys;
        const {
          _parsed: p
        } = this;
        if (!p.params) p.params = this.getParams(p.pathname);
      }
    }
  }
   ...
   * @param {string} path The path to extract route parameters from.
   * @returns {object|undefined} A object containing key-value pairs of the extracted route parameters.
   */
  getParams(path) {
    const {
      _regexp,
    ...
    if (_regexp && _pathkeys instanceof Array) {
      return FastHttpRequest._getParams(_regexp, _pathkeys, path);
    }
  }
  ...
  static getParams(routePath, path) {
    if (path) {
      const keys = [];
      const regexp = pathToRegexp(routePath || path, keys);
      return FastHttpRequest._getParams(regexp, keys, path);
    }
  }
  ...
  static _getParams(regexp, pathkeys, path) {
    if (pathkeys instanceof Array) {
      const results = regexp.exec(path);
      if (results instanceof Array) {
        const params = {};
        for (let i = 0; i < pathkeys.length; i++) {
          const name = pathkeys[i]["name"];
          // first element in results is the full match; skip it
